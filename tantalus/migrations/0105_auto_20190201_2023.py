# -*- coding: utf-8 -*-
# Generated by Django 1.11.14 on 2019-02-01 20:23
...
def migrate_datasets(apps, schema_editor):
    ...
    for sequence in SequenceDataset.objects.all():
        ...
        dataset.samples.add(sequence.sample)
        dataset.libraries.add(sequence.library)
    ...
    print(Analysis.objects.all().count())

    count = 0
    for analysis in Analysis.objects.all():
        for sequence in analysis.input_datasets.all():
            analysis.inputs.add(Dataset.objects.get(name=sequence.name))
        for result in analysis.input_results.all():
            analysis.inputs.add(Dataset.objects.get(name=result.name))
        count +=1
        if not count % 100:
            print(count)
